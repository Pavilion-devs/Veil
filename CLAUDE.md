...
**Streaming mic STT ✅** — `runTurn {audio:true}`: app streams s16le/16k/mono PCM frames as `audio` events (on a `listening` handshake) → brain streaming-transcribes (`WHISPER_EN_TINY_Q8_0` + `VAD_SILERO_5_1_2`) → `partial`/`transcript` events → loop. `brainHandlers.js` (shared by `brainMain.js` + tests), `voice.js` `StreamingTranscriber`/`sttStreamFromPcm`, `node streamingAcceptance.js` (3/3; real-speech jfk hard-asserted, synthetic-command path also drives a real action). All suites green: acceptance 5/5, ipc 3/3, brainApi 2/2, streaming 3/3.
**Next:** the Swift "eyes & hands" app (teammate's lane per `PLAN.md §8`) replaces `refApp.js` — fork `farzaa/clicky`, build `AXTreeReader` + `Actuator` + the socket peer in `brain/PROTOCOL.md` (both APIs incl. streaming mic). Brain side unchanged: `brainMain.js` is the real entrypoint. Validate with `node runTurn.js --socket …` then push-to-talk. Remaining brain work: MedPsy + wallet skills (Phase 4), P2P (Phase 5).
