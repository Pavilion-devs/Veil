...
**Health skill ✅ (hero lead, Phase 4)** — `query_health` action: OCR the on-screen lab doc (doctr, reading-order reconstructed from bboxes) → MedPsy-4B reasoning (strips `<think>`) → speak. `skills/health.js`, `fixtures/labs.{html,png}` (Chrome-rendered sample), `ocr`+`medpsy` roles in `qvac.js`, MockHands `showDocument()` (Preview/OCR case). `node healthAcceptance.js` (2/2: OCR finds all 5 abnormals, MedPsy flags diabetes/lipids/kidney/liver). All 5 suites green (15 tests): acceptance 5/5, ipc 3/3, brainApi 2/2, streaming 3/3, health 2/2.
**Next:** the Swift "eyes & hands" app (teammate's lane per `PLAN.md §8`) replaces `refApp.js` — build `AXTreeReader` + `Actuator` + the socket peer in `brain/PROTOCOL.md`. Brain side unchanged: `brainMain.js` is the real entrypoint. Remaining brain work: wallet skill (Phase 4, needs real Chrome grounding), P2P (Phase 5).
