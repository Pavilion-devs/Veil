...
**P2P tiered intelligence ✅ scaffold (Phase 5)** — `p2p.js` `PeerLink` registers a delegated `plannerBig` (`loadModel({delegate:{providerPublicKey, fallbackToLocal:true}})`); orchestrator escalates to the peer on struggle + emits `delegate` (opt-in `peer` param; `VEIL_PEER_KEY` in `brainMain.js`). `peerProvider.js` = teammate's entrypoint. `node p2pAcceptance.js` (4/4: provider, offline-detect, **local fallback**, routing — single-machine). Cross-machine offload needs the peer's box.
**Wallet skill ✅ (hero "money closes", Phase 4)** — `wallet_send` action (amount/asset/to): fill a devnet send form via grounding, then a **MANDATORY voice-confirm gate** — speak amount/recipient/network, require an explicit spoken "confirm"; **fail-closed** (no confirm capability → never sends). Defense-in-depth: the orchestrator **blocks any direct click on a wallet Send/Confirm** so the gate can't be bypassed. `skills/wallet.js`, `mockWallet.js`, `parseConfirmation`, devnet address book. `node walletAcceptance.js` (6/6: confirmed→sent, **declined→NOT sent**, fail-closed, loop). DEVNET only, keys on-device. **All 7 suites green (25 tests): acceptance 5/5, ipc 3/3, brainApi 2/2, streaming 3/3, health 2/2, p2p 4/4, wallet 6/6.**
**Next:** the Swift "eyes & hands" app (teammate's lane per `PLAN.md §8`) replaces `refApp.js` — build `AXTreeReader` + `Actuator` + the socket peer in `brain/PROTOCOL.md`. Brain side unchanged: `brainMain.js` is the real entrypoint. Remaining: wallet **grounding on real Chrome/Phantom** (OCR-first §9, needs the Swift app) + wiring the confirm gate to live mic capture; P2P cross-machine; demo/artifacts.
