...
| **4. Hero skills** | Jun 12–13 | Health (MedPsy labs) + money (devnet wallet + confirm). **Health ✅: `query_health` → doctr OCR → MedPsy → speak (`healthAcceptance.js` 2/2). Money ✅ (brain side): `wallet_send` → fill form → mandatory voice-confirm gate (fail-closed) → submit; manual-submit blocked for safety (`skills/wallet.js`, `walletAcceptance.js` 6/6).** Real Chrome/Phantom grounding (OCR-first §9) pending the Swift app. |
